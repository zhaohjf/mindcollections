...
public class ClosuresDemo {
     ...
     * 可修改的自由变量不是一个好的实践！
     *
     * lambda表达式允许修改自由变量后，在逻辑上会造成一定混淆，同时也有可能会引发一些不必要的问题，而且引发的问题很让人难以察觉。
     *
     */
     ...
     * 总结
     *
     * 相比于JavaScript，在Java中实现是一个功能受限的闭包，它不允许修改自由变量的值。
     *
     * 之所以有这种不同，是因为在JavaScript中，闭包是它最基本的一种机制，这种机制用来定义和创建对象；而在Java中，类的加载和创建是由另一种
     * 方式来进行的。
     *
     */
}
